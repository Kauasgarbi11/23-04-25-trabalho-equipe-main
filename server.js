...
const pool = mysql.createPool({
    ...
    user: 'root',
    password: 'senac@02',
    database: '`cheirinho_de_bolo`.`cadastro`'
});
        ...
        switch (tipo) {
            case 'cadastro':
                var [rows, fields] = await pool.query(
                    "insert into `cheirinho_de_bolo`.`cadastro` (`nome`, `email`, `senha`) values (?, ?, ?);",
                    [nome, email, senha]
                );
            ...
            case 'login':
                var [rows, fields] = await pool.query(
                    "select * from `cheirinho_de_bolo`.`cadastro` where `nome` = ? and `email` = ? and `senha` = ?;",
                    [nome, email, senha]
                );
                if (rows.length == 1) {
            ...
            case 'leitura':
                ...
                var strSql = "select * from `cheirinho_de_bolo`.`cadastro` where" + 
                ...
                break;
            case 'atualizacao':
                var strSql = "select * from `cheirinho_de_bolo`.`cadastro`;";
                var [rows, fields] = await pool.query(strSql);
                if (rows.length > 0) {
                    res.json({ 
            ...
            case 'atualizar':
                ...
                var strSql = "update `cheirinho_de_bolo`.`cadastro` set " + 
